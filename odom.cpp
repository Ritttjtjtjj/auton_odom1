...
void Odom::pointTurn(double targetDeg, double maxTimeSeconds) {
    ...
    // Normalize the target to 0–360 range
    double target = fmod(targetDeg, 360.0);
    if (target < 0) target += 360.0;

    std::uint32_t start = pros::millis();

    while (true) {
        update(); // keeps x,y,heading updated
        ...
        // Point turn = wheels go opposite directions
        setDrivePower(-power, power);

        if (fabs(error) < 1.5) break; // within ~1.5 degrees
        if (pros::millis() - start > maxTimeSeconds * 1000) break;
